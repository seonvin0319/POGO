...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_critic", action="store_true")

    # GFlow (Baseline)
    parser.add_argument("--alpha", type=float, default=1.0)
    parser.add_argument("--entropy_weight", type=float, default=0.01)
    parser.add_argument("--w2_weight", type=float, default=0.5)
    parser.add_argument("--w2_mul", type=float, default=1.0)
    ...
    parser.add_argument("--load_prefix", type=str, default="",
                   help="예: ./POGO-W/models/POGO_Two-Step_hopper-medium-v2_0  (확장자 없이 prefix)")

    args = parser.parse_args()

    if not args.two_step:
        file_name = f"POGO-W_One-Step_{args.env}_{args.seed}"
    else:
        file_name = f"POGO-W_Two-Step_{args.env}_{args.seed}"
    
    print("---------------------------------------")
    print(f"Two-Phase POGO-W (agent switch with {args.w2_mul} * w2_weight), Env: {args.env}, Seed: {args.seed}")
    print("---------------------------------------")

    os.makedirs("./POGO-W/results", exist_ok=True)
    if args.save_model:
        os.makedirs("./POGO-W/models", exist_ok=True)
    if args.save_critic:
        os.makedirs("./critics", exist_ok=True)

    # MID_DIR = "./models_mid_w"
    ...
    if args.wandb:
        if args.run_name:
            ...
            if args.two_step:
                run_name = f"{args.env}_POGO-W_Two-Step_{args.w2_mul}*w2_seed{args.seed}"
            else:
                run_name = f"{args.env}_POGO-W_One-Step_seed{args.seed}"
        init_kwargs = dict(
            project=args.wandb_project,
            name=run_name,
            group=args.env,
            tags=["POGO-W_Two-Step"],
            config=vars(args),
        )
    ...
    if args.load_model != "":
        ...
        agentA.load(f"./models_mid_w/{policy_file}")
    ...
    for t in range(split_step):
        ...
        if (t + 1) % args.eval_freq == 0:
            ...
            if args.save_model:
                agentA.save(f"./POGO-W/models/{file_name}")
                if d4rl_score > best_eval:
                    best_eval = d4rl_score
                    agentA.save(f"./POGO-W/models/{file_name}_best")
    ...
    if remaining > 0:
        ...
        agentB = GFlow_W2_Refine(
            ...
            policy_freq=args.policy_freq,
            alpha=args.alpha,
            w2_weight=args.w2_weight * args.w2_mul,
            entropy_weight=args.entropy_weight,
            learning_rate=args.learning_rate,
        )
        ...
        for t2 in range(remaining):
            ...
            if (global_step + 1) % args.eval_freq == 0:
                ...
                if args.save_model:
                    agentB.save(f"./POGO-W/models/{file_name}")
                    if d4rl_score > best_eval:
                        best_eval = d4rl_score
                        agentB.save(f"./POGO-W/models/{file_name}_best")
    ...
    if args.save_critic:
        active_policy.save(f"critics/POGO-W_One-Step_{args.env}_{args.seed}")
